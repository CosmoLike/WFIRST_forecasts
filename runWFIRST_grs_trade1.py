...
import ctypes
n_trade = 1
chainfile = "/Users/timeifler/Dropbox/cosmolike_store/WFIRST_forecasts/chains/trade%d_kmin0001" %(n_trade)
print chainfile
...
def likelihood(p):
	like=loglike(p[0],p[1],p[2],p[3],p[4],p[5],p[6],1.19,1.3,1.44,1.57,1.70,1.82,1.93,290.,290.,290.,290.,290.,290.,290.,0.001,0.0,0.24)
	if like < -1.0e+10:
		return -np.inf
	return like
...
ndim=7
nwalker=500
sampler = emcee.EnsembleSampler(nwalker, ndim, likelihood,threads=1)
starting_point=np.array([0.3156,0.831,0.9645,-1.,0.,0.0491685,0.6727])
std=np.array([0.05,0.05,0.02,0.1,0.2,0.002,0.03])
